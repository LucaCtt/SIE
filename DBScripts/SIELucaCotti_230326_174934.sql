...
create table `eventoinevidenza` (
   `eventopubblico_oid`  integer  not null,
  primary key (`eventopubblico_oid`)
);
...
-- EventoPubblico [ent20]
create table `eventopubblico` (
...
alter table `eventoinevidenza`   add index fk_eventoinevidenza_eventopubb (`eventopubblico_oid`), add constraint fk_eventoinevidenza_eventopubb foreign key (`eventopubblico_oid`) references `eventopubblico` (`evento_oid`);


-- GEN FK: EventoPubblico --> Evento
alter table `eventopubblico`   add index fk_eventopubblico_evento (`evento_oid`), add constraint fk_eventopubblico_evento foreign key (`evento_oid`) references `evento` (`oid`);
